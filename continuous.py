...
class TanhNormal(FasterTransformedDistribution):
    ...
    def get_sampled_mean(self):
        """Computes an estimation of the mean by averaging samples."""
        N = 100_000
        X = self.sample((N,))
        return X.mean(0)
    ...
    @torch.enable_grad()
    def get_mean(self):
        """Computes an estimation of the mean using the Adam optimizer."""
        # Get starting point
        m = self.sample((1000,)).mean(0)
        m = torch.nn.Parameter(m.clamp(self.low, self.high).detach())
        optim = torch.optim.Adam((m,), lr=1e-2)
        ...
        for _ in range(200):
            ...
            X = self_copy.sample((1000,))
            # Compute the loss as Mean Squared Error between samples and m
            loss = ((X - m) ** 2).mean()
            loss.backward()
            mc = m.clone().detach()
            m.grad.clamp_max_(1)
